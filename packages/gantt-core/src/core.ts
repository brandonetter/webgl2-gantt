...
class FloatInstanceBuffer {
  ...
  constructor(public readonly stride: number, initialCapacity = 256) {
...
export class GlyphInstanceWriter extends FloatInstanceBuffer implements GlyphSink {
...
export function createCamera(viewportWidth = 1280, viewportHeight = 720): CameraState {
  return {
    scrollX: 0,
    scrollY: 0,
    zoomX: 1,
...
export function resizeCamera(camera: CameraState, viewportWidth: number, viewportHeight: number): CameraState {
  return {
    ...camera,
    viewportWidth,
    viewportHeight,
...
export function panCamera(camera: CameraState, deltaScreenX: number, deltaScreenY: number): CameraState {
  return {
    ...camera,
...
export function worldToScreen(camera: CameraState, worldX: number, worldY: number): [number, number] {
  return [
...
export function screenToWorld(camera: CameraState, screenX: number, screenY: number): [number, number] {
  return [
...
export function buildTaskIndex(tasks: GanttTask[]): TaskIndex {
  ...
  for (const task of tasks) {
    ...
    rowMaxDuration[task.rowIndex] = Math.max(rowMaxDuration[task.rowIndex], task.end - task.start);
  }

  for (const row of rows) {
    row.sort((left, right) => left.start - right.start || left.end - right.end || left.id.localeCompare(right.id));
  }
...
export function computeVisibleRowRange(
  ...
  const start = clamp(Math.floor(camera.scrollY / rowPitch) - overscanRows, 0, Math.max(0, rowCount - 1));
  const end = clamp(
    Math.ceil((camera.scrollY + camera.viewportHeight / camera.zoomY) / rowPitch) + overscanRows,
    0,
...
export function computeVisibleTimeWindow(camera: CameraState, overscanPx = 220): ViewWindow {
  return {
...
function alignTimeStep(daySerial: number, step: TimeAxisStep): number {
  ...
  const alignedYear = Math.floor(date.getUTCFullYear() / step.count) * step.count;
...
function advanceTimeStep(daySerial: number, step: TimeAxisStep): number {
  ...
  if (step.unit === 'month') {
    return utcMsToDaySerial(Date.UTC(date.getUTCFullYear(), date.getUTCMonth() + step.count, 1));
  }
...
export function taskWorldRect(task: GanttTask, rowPitch: number, barHeight: number): {
  x: number;
  y: number;
  w: number;
  h: number;
...
export function pickTaskAtPoint(
  ...
  const startIndex = Math.max(0, lowerBound(rowTasks, timeWindow.start - index.rowMaxDuration[worldRow], (task) => task.start));
...
function appendLabelWithShadow(
  ...
  layout.appendText(glyphs, text, x + shadowOffsetX, baselineY + shadowOffsetY, fontPx, shadowColor, maxWidth);
...
function appendVisibleLine(
  ...
  if (!lineIntersectsViewport(sx1, sy1, sx2, sy2, camera.viewportWidth, camera.viewportHeight, padding)) {
    return false;
  }
...
function buildRoundedSegments(points: Point[], radius: number): DependencySegment[] {
  if (points.length < 2) {
    return [];
  }
  ...
  for (let i = 1; i < points.length - 1; i += 1) {
    ...
    const isCorner = Math.abs(inDir.x - outDir.x) > 0.001 || Math.abs(inDir.y - outDir.y) > 0.001;
    ...
    const entry = { x: curr.x - inDir.x * cornerRadius, y: curr.y - inDir.y * cornerRadius };
    const exit = { x: curr.x + outDir.x * cornerRadius, y: curr.y + outDir.y * cornerRadius };
    ...
    for (let step = 1; step <= steps; step += 1) {
      ...
      segments.push({ x1: previousPoint.x, y1: previousPoint.y, x2: nextPoint.x, y2: nextPoint.y });
...
function buildDependencySegments(
  ...
  const sourceRect = taskWorldRect(sourceTask, config.rowPitch, config.barHeight);
  const targetRect = taskWorldRect(targetTask, config.rowPitch, config.barHeight);
  const sourceAnchor = {
  ...
  const verticalOffset = Math.max(8 / camera.zoomY, (config.rowPitch - config.barHeight) * 0.5 + 2 / camera.zoomY);
  ...
  if (targetIsBelow && targetIsToRight) {
    ...
    const laneY = Math.max(sourceAnchor.y + verticalOffset, targetRect.y + targetRect.h + 6 / camera.zoomY);
    points.push({ x: sourceAnchor.x, y: laneY });
    points.push({ x: targetAnchor.x, y: laneY });
    points.push({ x: targetAnchor.x, y: targetAnchor.y });
  } else {
    const detourX = Math.max(sourceRect.x + sourceRect.w, targetRect.x + targetRect.w) + 18 / camera.zoomX;
    const laneY = Math.max(sourceAnchor.y + verticalOffset, targetRect.y - 6 / camera.zoomY);
    points.push({ x: sourceAnchor.x, y: laneY });
  ...
  const segments = buildRoundedSegments(points.concat(targetAnchor), cornerRadius);
  if (segments.length === 0) {
    return [];
  }

  const targetScreen = worldToScreen(camera, targetAnchor.x, targetAnchor.y);
  const previousScreen = worldToScreen(camera, segments[segments.length - 1].x1, segments[segments.length - 1].y1);
  ...
  segments.push({ x1: leftWorld[0], y1: leftWorld[1], x2: targetAnchor.x, y2: targetAnchor.y });
  segments.push({ x1: rightWorld[0], y1: rightWorld[1], x2: targetAnchor.x, y2: targetAnchor.y });
  return segments;
}
...
export function pickDependencyAtPoint(
  ...
  for (const path of frame.dependencyPaths) {
    for (const segment of path.segments) {
      ...
      const distance = distanceToSegmentSquared(screenX, screenY, x1, y1, x2, y2);
...
export function buildFrame(
  ...
  const backgroundSolids = new SolidInstanceWriter(Math.max(1024, index.rowCount * 4));
  const foregroundSolids = new SolidInstanceWriter(Math.max(1024, index.rowCount * 8));
  const backgroundLines = new LineInstanceWriter(Math.max(1024, index.rowCount * 4));
  const dependencyLines = new LineInstanceWriter(Math.max(1024, index.rowCount * 8));
  const glyphs = new GlyphInstanceWriter(Math.max(2048, index.rowCount * 24));
  const dependencyPaths: DependencyPath[] = [];

  const rowRange = computeVisibleRowRange(camera, config.rowPitch, index.rowCount, config.overscanRows);
  const window = computeVisibleTimeWindow(camera, config.overscanPx);
  const labelTier = chooseLabelTier(camera.zoomX);
  const gridStep = chooseGridStep(camera.zoomX);
  const gridColor: [number, number, number, number] = [0.12, 0.15, 0.2, 0.28];
  const separatorColor: [number, number, number, number] = [0.14, 0.17, 0.22, 0.85];
  const textColor: [number, number, number, number] = [0.96, 0.98, 1.0, 0.96];
  const axisTextColor: [number, number, number, number] = [0.8, 0.86, 0.93, 0.94];
  ...
  const gridBottom = Math.min(scene.rowLabels.length, rowRange.end + 1) * config.rowPitch;
  ...
  for (let time = alignTimeStep(gridLeft, gridStep); time <= gridRight; time = advanceTimeStep(time, gridStep)) {
    backgroundLines.appendLine(time, gridTop, time, gridBottom, gridColor[0], gridColor[1], gridColor[2], gridColor[3], 1);
    gridLineCount += 1;
  }
  ...
  for (let row = rowRange.start; row <= rowRange.end; row += 1) {
    ...
    backgroundSolids.appendRect(window.start, rowY, window.end - window.start, config.rowPitch, r, g, b, a, 0, 0);
    ...
    const rowStartIndex = Math.max(
      0,
      lowerBound(rowTasks, window.start - index.rowMaxDuration[row], (task) => task.start),
    );
    ...
    for (let i = rowStartIndex; i < rowTasks.length; i += 1) {
      ...
      const pluginStyle = renderState.taskStyleResolver?.({ task, selected, hovered }) ?? null;
      const fill = pluginStyle?.fill ?? taskFillColor(task, renderState.selectedTaskId, renderState.hoveredTaskId);
      const emphasis = pluginStyle?.emphasis ?? defaultEmphasis;
      const laneY = rowY + (config.rowPitch - config.barHeight) * 0.5;

      if (task.milestone) {
        const size = config.milestoneSize * (selected ? 1.15 : hovered ? 1.05 : 1);
        ...
        if (labelTier.enabled) {
          ...
          const baseline = screenY + (screenH - textBoxHeight) * 0.5 + atlas.ascender * scale;
          const fullLabelWidth = layout.measure(task.label, labelTier.fontPx);
          const visibleLeft = clamp(screenX, 0, camera.viewportWidth);
          const visibleRight = clamp(screenX + screenW, 0, camera.viewportWidth);
          const visibleWidth = Math.max(0, visibleRight - visibleLeft);
          const insideWidth = Math.max(0, visibleWidth - config.labelPadding * 2);
          const fitsInside = visibleWidth >= labelTier.minBarWidth && fullLabelWidth <= insideWidth;

          if (fitsInside) {
            const centeredLabelX = visibleLeft + (visibleWidth - fullLabelWidth) * 0.5;
            const minLabelX = visibleLeft + config.labelPadding;
            const maxLabelX = visibleRight - config.labelPadding - fullLabelWidth;
            ...
            const availableWidth = Math.max(0, camera.viewportWidth - labelX - config.labelPadding);
            const label = layout.fit(task.label, availableWidth, labelTier.fontPx);
            if (label.length > 0) {
              appendLabelWithShadow(
                glyphs,
                layout,
  ...
  for (let tick = alignTimeStep(camera.scrollX, gridStep); tick <= viewportRight; tick = advanceTimeStep(tick, gridStep)) {
  ...
  for (const task of scene.tasks) {
    ...
    for (const depId of task.dependencies ?? []) {
      ...
      let emitted = false;
      const segments = buildDependencySegments(predecessor, task, camera, config);
  ...
  return {
    ...
    stats: {
      ...
      rowEnd: rowRange.end,
      visibleRows: rowRange.end >= rowRange.start ? rowRange.end - rowRange.start + 1 : 0,
      visibleTasks,
      visibleDependencies,
      glyphCount: glyphs.count,
      gridLineCount,
      rowBandCount: rowRange.end >= rowRange.start ? rowRange.end - rowRange.start + 1 : 0,
    },
  };
}
